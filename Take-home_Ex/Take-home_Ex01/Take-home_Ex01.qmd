...
:::
#### 3.1.3 Compute KDE with adaptive bandwith
...
```{r}
F_9601.csr <- envelope(biz_9601_ppp, Fest, correction = "all", nsim = 999)
```
### Plot simulation
```{r}
par(bg = "#EEEEEE")
...
```{r}
L_9602.csr <- envelope(biz_9602_ppp, Lest, nsim = 99, rank = 1, glocal = TRUE)
```
...
F_9601_quarter = Fest(biz_9601_quarter_ppp)
tims <- c(1,2,3,4,5,6)
par(mfcol = c(1,3), bg = "#EEEEEE")
for(i in tims){
  plot(F_9601, i,
       override.par = FALSE,
       fix.range = TRUE, main = paste("Laundry Services Spatial Point Process at Quarter", i
     ), col = terrain.colors(256))
}
...
## Beauty Services - month/G-function


## Dental Services - quarter/L
