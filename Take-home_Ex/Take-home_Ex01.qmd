...
Now we will convert the tibble data frame to a `sf` object. ::: panel-tabset
...
::: notegoals-container
Laundry service R = 0.59189 & p-value = 0.01 shows **the distribution is not random**. As R value is between 0.5-1.0, there are some clustering in residential areas. It is more dispersed to serve neighbourhood needs, but it is not random.
:::
:::::
...
::: notegoals-container
Beauty services R = 0.36 & p-value = 0.01 indicates that they are not random. The R value is 0.0-0.5, showing **strong clustering**, meaning that the points are very close together.
:::
:::::
...
::: notegoals-container
Dental service R = 0.49043 & p-value = 0.01, so the distribution of dental businesses are not random. As R value is around 0.5, it shows moderate to strong clustering. This is possibly due to medical professional buildings or proximity to hospitals.
:::
:::::
::::::::::::
...
::: notegoals-container
The visualisation above shows the new laundry businesses spatial patters by quarter from 2024-Q1 to 2025-Q2. Several dustubct temporal trends are evident. - **Northern areas**: After notable laundry service establishments in northern Singapore during the first half of 2024 (Q1-Q2), new business openings in this region significantly decreased, with zero to very few establishments in subsequent quarters.
...
**Temporal Growth Patterns**: 2024-Q1 shows strong initial central concentration. 2024-Q2&Q3 demonstrates market consolidation with maintained central focus. 2024-Q4 onwards exhibits notable expansion into residential areas, particularly in eastern and northeastern regions. The most significant development is the progressive density increase in eastern and northeastern residential areas from 2024-Q4 through 2025-Q2. This expansion suggests market maturation enabling viable residential location strategies, increased demand in these areas, and business model evolution toward neighborhood-based services.
:::
:::::
...
**Central Region Concentration**: New dental services demonstrate central clustering during 2024-Q1, 2024-Q3 and 2025-Q2. This concentration reflects the specialised nature of dental healthcare services and their dependence on medical districts, accessibility, and infrastructure. Western Region (Holland area) shows sporadic activity only in Q1, with no subsequent expansion, suggesting either market testing or unsuccessful location strategy.
...
```{r}
F_9601.csr <- envelope(biz_9601_ppp, Fest, correction = "all", nsim = 39)
```
...
```{r}
G_86204.csr <- envelope(biz_86204_ppp, Gest, nsim = 39)
```
...
::: notegoals-container
The observed line is completely above the envelope, and shows a very strong clustering pattern with highly statistically significant. With out obervation earlier on the density of dental businesses in the maps, the pattern suggests dental services form tight professional clusters rather than being distributed based on residential density alone.
:::
:::::
::::::::::::
...
::: notegoals-container
During the 6 quarters of observation, most of the time the F*km* line is consistenly below F*pois* line, except 2024-Q2 and 2024-Q4. This means new laundry services tend to cluster when they open.
...
```{r}
F_9601_quarter.csr <- lapply(biz_9601_quarter_split, function(x) envelope(x, Fest, nsim = 39))
```
...
::: notegoals-container
-   Based on the test result, we found no statistically significant deviation from randomness in any quarter. Therefore, earlier on we observed that F_km below F_pois for 2024-Q4 is not strong enough to be statistically significant.
-   This means weak clustering tendencies, meaning that new laundry services show mild clustering preferences but not strong ones. In addition, the spatial choices are more opportunistic than strategically clustered.
-   For business, laundry services seem to have less rigid location strategies compared to other service types.
:::
:::::
...
```{r}
L_9602_quarter.csr <- lapply(biz_9602_quarter_split, function(x) envelope(x, Lest, nsim = 39))
```
...
```{r}
G_86204_quarter.csr <- lapply(biz_86204_quarter_split, function(x) envelope(x, Gest, nsim = 39))
```
