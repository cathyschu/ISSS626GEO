...
```{r}
# Join PM_weekday to your hexagon spatial object
hexagon_pm <- hexagon %>%
  left_join(PM_weekday, by = "HEX_ID")

# Check if join worked
names(hexagon_pm)  
head(hexagon_pm$PM_TRIPS)  

# Handle any NA values - we fill in zero for hexagons with no trips
...
tm_shape(hexagon_pm) +
  tm_fill("PM_TRIPS", palette = "Purples", fill_alpha = 1) +
...
pm_tm_sq <- pm_tm_qt <- tm_shape(mpsz) +
  tm_polygons(fill = "#A1C2BD", alpha = 0.3) +
  tm_shape(hexagon_pm) +
...
tm_shape(hexagon_pm) +
  tm_fill("PM_TRIPS", palette = "Purples", fill_alpha = 1,
         style = "quantile",  # Equal number of hexagons in each color group
          n = 6) +
  tm_borders(col = "grey20", lwd = 0.5) +
  tm_layout(bg.color = "#D0DDD0",
            outer.margins = c(0.01, 0.01, 0.01, 0.01),
            main.title = "PM Peak Hour Trips per Quantile",
            legend.position = c("right", "bottom"),
            frame = FALSE)
