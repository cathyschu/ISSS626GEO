...
Instead of using planning zones or subzones determined by Singapore administrations, we will use prepare hexagon data to perform geospatial analysis.
...
glimpse(AM_trips_hex, 5)
glimpse(PM_trips_hex, 5)
glimpse(weekend_trips_hex, 5)
...
Summary Report: There are 826 regions (hexagons) in the Singapore map. The most connected area unit has 6 neighbors (hexagon has 6 edges). There is one unit with no neighbors, and the Hex ID is 826.
...
-   p-value = 0.001 indicates that it is highly significant (little chance that this pattern is random) and confirms the analytical test result from the Moran's I test.
...
-   PM weekday clustering seems weaker. It may be because afternoon trips are more varied. People start trips from more varied locations (workplaces, shopping, schools, scattered throughout the city), so the trips origins are more spread out across commercial business and retail areas. In addition, afternoon has more staggered departures across different times. For examples, some may leave office earlier than 5pm to avoid the traffic or fetch children from schools.
...
**Prepare data for visualisation** ::: panel-tabset
...
**Plot the LISA map below** ::: panel-tabset
...
Join EHSA results with geometry ::: panel-tabset
