...
This assignment from the Geospatial course by Prof. Kam provided us an opportunity to conduct explanatory or predictive moderling analysis. Given that the author worked on an explanatory moderling in Data Analytics Lab, she will focus on Predictive Modeling for this topic.
...
<placeholder> analysis showing when spatial ensemble methods fail to add value!
...
Below table shows the packages used for this study and their descriptions, including links to more details to the package.
...
We will make use of the following daa sets.
...
-   **Further processing for street names** We do further processing for the data as the street names with Saint are usually written as St.
...
-   **Geocoding HDB Resale Data** We use OneMap Geocode API: - <https://www.onemap.gov.sg/apidocs/.> - [ttps://www.onemap.gov.sg/apidocs/reverseGeocode](ttps://www.onemap.gov.sg/apidocs/reverseGeocode)
...
Amber Mansions - demolished in 1984 Bedok Point Capitol Centre Ellenborough Market Funan DigitaLife Mall - duplicate Great World - duplicate GV Yishun - cinema Jurong Entertainment Centre - demolished 2010 Shaw Tower - demolished Specialists' Shopping Centre - redeveloped as Orchard Gateway Tanglin Shopping Centre - CLOSED The Golden Mile - CLOSED The Verge - closed Woodland Mall - change to Woods square mall
...
For this study, we will use the entire dataset of `HDB_master` during the period of July 2024 to June 2025. Therefore, we will split the data to 2 sets. In this step we will split the `HDB_master` to the training data (Jul 2024 - June 2025) and testing data (July - Sep 2025).
...
floor_level + plan_area + floor_area_sqm + flat_model + geometry + remaining_lease_mths_num + PROX_HKCENTER + PROX_SUPERMKT + PROX_CLINIC + PROX_ELDERCARE + PROX_MALLS + PROX_SPORTS + PROX_CC + PROX_PARK + PROX_MRT + PROX_CBD + PROX_BIZPK + WITHIN_1km_TOPPRISCH_BINARY + WITHIN_500m_HKC_BINARY + WITHIN_350m_KINDG + WITHIN_350m_CHILDC + WITHIN_350m_BUS + WITHIN_1km_PRISCH,
...
In this section, we will calibrate Geographical Weighted Random Forest Model with the training data. \### 5.4.1 Bandwidth selection We will use GWR Bandwidth for GWRF: `bw.adaptive`
...
We also plot the metrics in

Which model predicts best? Performance on unseen data

### 7.4 Best model selection

-   Best overall model GWR (lowest RMSE = 38,028; lowest MAE = 28,750, highest R² = 0.9587)

-   Improvement over baseline Comparing spatial models (GWR, gwRF) vs non-spatial (MLR, RF), all 3 metrics show that the spatial models clearly significantly outperform non-spatial models. MLR is the bottom performer among the 4 models. This validates that spatial heterogeneity is important, so using geographical weighted methods much better predicts the HDB resle price.

-   Model complexity trade-off When we run the models using 5,000 datasets for training and approximately 3,000 datasets for testing, GWR modeling required the most time - around 15-40 minutes. The rest of the modeling did not take noticeable long time to complete training and testing. This is given that the author's laptop specs is premium for machine learning projects. Although GWR is more computational expensive, it gives the best performance. If resource is scarcer, gwRF is recommended
...
The results reveal important insights about spatial modeling for HDB resale prices: Spatial heterogeneity is significant: GWR substantially outperformed all other models, achieving the lowest RMSE (38,028) and highest R² (0.9587). This demonstrates that locally-varying relationships between property characteristics and prices are crucial for accurate prediction in Singapore's HDB market. Different neighborhoods exhibit different pricing dynamics that cannot be captured by global models. Random Forest cannot fully substitute for spatial modeling: Despite its flexibility, RF (RMSE=41,950) could not match GWR's performance. While RF can learn complex patterns, it lacks the explicit spatial weighting mechanism that allows GWR to adapt coefficients by location. The \~4,000 SGD difference in RMSE represents practically significant prediction error (\~10% improvement with GWR). Over-localization problem in gwRF: Adding spatial weighting to Random Forest actually degraded performance below standalone RF. With bandwidth=48 neighbors, each local RF trains on insufficient data, leading to overfitting. The hybrid approach (60% local, 40% global) partially recovered performance but still couldn't match either GWR or RF. Model recommendations:

For best prediction accuracy: Use GWR - it provides superior performance and spatial interpretability For computational efficiency: Use RF as a faster alternative with acceptable accuracy Avoid: Fully local gwRF due to overfitting and poor performance despite high computational cost Best performing model Spatial vs non-spatial comparison Variable importance insights Limitations

### 8.1 Key findings

We tested a hybrid gwRF approach (60% local, 40% global weights) to address potential over-localization. While the hybrid model outperformed the fully local gwRF, it remained inferior to both GWR and standalone RF. This confirms that:

Pure local modeling with bandwidth=48 is suboptimal for ensemble methods Even with global information, gwRF cannot match the performance of purpose-built spatial (GWR) or global ensemble (RF) models

Conclusion: gwRF, whether fully local or hybrid, does not provide a useful middle ground for this dataset and should not be preferred over GWR (for accuracy) or RF (for efficiency).

### 8.2 gwRF bandwidth sensitivity analysis

We conducted a bandwidth sensitivity analysis testing values from 48 to 200 neighbors. Results showed a positive trend: as bandwidth increased, gwRF performance improved substantially (RMSE decreased from 46,306 at BW=48 to 41,396 at BW=200). However, even at the optimal bandwidth of 200, gwRF still could not outperform the simpler Random Forest model (RMSE=41,950). This finding reveals a fundamental limitation: as gwRF's bandwidth increases to improve performance, it essentially converges toward global modeling, negating the theoretical advantage of local spatial weighting. The bandwidth sensitivity analysis confirms that:

Small bandwidths lead to over-localization and unstable local models Large bandwidths approach global modeling behavior At no bandwidth does gwRF provide a meaningful advantage over either GWR (for spatial modeling) or RF (for global ensemble modeling)

Conclusion: gwRF occupies an awkward middle ground - when localized it underperforms due to insufficient data, and when globalized it merely approximates RF at higher computational cost. \### 8.2 Limitations & Future work
...
```{r}
# Plot results
ggplot(gwrf_results, aes(x = Bandwidth, y = RMSE)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 41950, color = "blue", linetype = "dashed") +  # RF
  geom_hline(yintercept = 38028, color = "red", linetype = "dashed") +   # GWR
  labs(title = "gwRF Performance by Bandwidth",
       subtitle = "Blue = RF, Red = GWR") +
  theme_minimal()
```
