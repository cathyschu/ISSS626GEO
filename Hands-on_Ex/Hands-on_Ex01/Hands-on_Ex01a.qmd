...
# can just do the process in one chunk
#preschool = st_read("data/geospatial/PreSchoolsLocation.kml") %>%
...
```{r}
# always use to check the projection first.
st_crs(mpsz)
```
...
1.  st_buffer() of sf package is used to compute the 5-meter buffers around cycling paths. Then, calculate the area of the buffers.
...
2.  Lastly, `sum()` of Base R will be used to derive the total land involved
...
::: markgoals-container
[st_intersects()](https://r-spatial.github.io/sf/reference/geos_binary_pred.html) is different from [st_intersection()](https://r-spatial.github.io/sf/reference/geos_binary_ops.html).
:::
:::::
