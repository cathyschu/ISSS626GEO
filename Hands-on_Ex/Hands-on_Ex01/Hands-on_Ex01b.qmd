...
The code chunk below uses the *st_read()* function of **sf** package to import `MP14_SUBZONE_WEB_PL` shapefile into R as a simple feature data frame called `mpszkml`.

```{r}
mpszkml <- st_read("data/geospatial/MasterPlan2019SubzoneBoundaryNoSeaKML.kml")
```
...
mpszkml <- mpszkml %>%
  ...
  relocate(geometry, .after = last_col()) #move the col to the last col
```
...
```{r}
mpsz_pop2024 <- left_join(mpszkml, popdata2024,
                          by = c("SUBZONE_N" = "SZ"))
```
...
tm_shape(mpsz_pop2024) +
  ...
  tm_scalebar() +
  tm_grid(alpha =0.2) + #projection on the fly
...
::: markgoals-container
-   The default interval binning used to draw the choropleth map is called “pretty”. A detailed discussion of the data classification methods supported by **tmap** is in sub-section 2.4.3.

-   The default colour scheme used is `blues3` of ColorBrewer. Here a theme is applied. More about the color scheme in sub-section 2.4.4.
...
With **tmap,** statistical chart and be incorporate into the map visualisation by using `fill.chat` argument of map layers and [**legend chart**](https://r-tmap.github.io/tmap/reference/tm_chart.html).
