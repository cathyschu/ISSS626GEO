...
pacman::p_load(sf, rgdal, spdep, tidyverse, tmap, corrplot, ggpubr, heatmaply, cluster, ClustGeo,
               factoextra, NbClust, psych, GGally, knitr)
```
...
First, we need to convert `shan_sf` into SpatialPolygonsDataFrame. This is because SKATER function only support **sp** objects such as SpatialPolygonDataFrame. Below we use [*as_Spatial()*](https://r-spatial.github.io/sf/reference/coerce-methods.html) of **sf** package to convert *shan_sf* into a SpatialPolygonDataFrame called *shan_sp*.
...
We can plot the neighbours list on shan_sp using the code chunk below.

Since we now can plot the community area boundaries as well, we plot this graph on top of the map. The first plot command gives the boundaries, followed by the plot of the neighbor list object, with coordinates applied to the original SpatialPolygonDataFrame (Shan state township boundaries) to extract the centroids of the polygons. These are used as the nodes for the graph representation.
...
Below is to compute the cost of each edge.
...
The minimum spanning tree is computed by mean of the [*mstree()*](https://r-spatial.github.io/spdep/reference/mstree.html) of **spdep** package. After computing the MST, we can check its class and dimension.
...
We can display the content of *shan.mst* below:
...
[**Note:**]{style="color: #B95E82;"} It is set to **one less than the number of clusters**. So, the value specified is **not** the number of clusters, but the number of cuts in the graph, one less than the number of clusters.
...
We can check the cluster assignment below:
...
Below we plot the newly derived clusters by using **SKATER** method.
...
For easy comparison purposed, let's place both the hierarchical clustering and spatially constrained hierarchical clustering maps next to each other.
...
Example code: [5.7.6 Computing proximity matrix](https://r4gdsa.netlify.app/chap05#computing-proximity-matrix)
...
Now we will plot the map of the newly delineated spatially constrained clusters.
...
Plot below is used to reveal the distribution of a clustering variable (i.e RADIO_PR) by cluster.
...
Past studies show that [parallel coordinate plot can be used to reveal clustering variables by cluster]{style="color: #B95E82;"} very effectively. In the code chunk below, [`ggparcoord()`](https://ggobi.github.io/ggally/reference/ggparcoord.html) of [**GGally**](https://ggobi.github.io/ggally/index.html) package

```{r}
#| fig-width: 8
#| fig-height: 4
#| fig-align: center
theme_set(theme_gray(base_size = 9, base_line_size = 0.3))
...
## 11 Reference
-   Kam, T. S. (2022, December 8). R for geospatial data science and analytics. https://r4gdsa.netlify.app/chap12#overview