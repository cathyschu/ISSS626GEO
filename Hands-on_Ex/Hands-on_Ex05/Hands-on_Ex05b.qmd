...
-   p_ii: p-value (**p \< 0.05**: Statistically significant clustering/outlier)

-   p_ii_sim: P-value (Simulated/Monte Carlo)

-   z_ii:the standard deviate of local moran statistic

-   Pr(): the p-value of local moran statistic

**Look at `ii`** to see if there's clustering (positive) or outliers (negative)

**Check `p_ii_sim`** to see if it's statistically significant (\< 0.05)
...
**Positive ii means:** Location has similar values to its neighbors (clustering) **Negative ii means:** Location has different values from neighbors (spatial outlier)
...
This is to show statistical significance based on Monte Carlo simulation. It is more accurate, especially for small sample sizes or skewed data.
...
::: markgoals-container
A new class called Insignificant has been added for records whereby p_ii_sim \> 0.05.
:::
:::::
...
In this section, we will compute adaptive distance weight matrix by using `st_knn()` and `st_weights()` of **sfdep** package.
...
## Gi\* with fix distance weights p-value

The choropleth shows there is evidence for both positive and negative Gi\* values. However, it is useful to consider the p-values for each of these values, as consider above.
